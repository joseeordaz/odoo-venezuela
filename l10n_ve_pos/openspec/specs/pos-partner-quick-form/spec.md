# pos-partner-quick-form Specification

## Purpose

Definir el formulario de contacto que el PdV usa en Odoo 19 — una vista form
reducida propia de `l10n_ve_pos`, derivada de `base.view_partner_form` en
`mode="primary"` sin modificarla — y la precarga de los campos de localización
venezolanos (país, estado, ciudad, municipio, parroquia, C.P.) con la dirección
de la compañía, para que el cajero no los teclee en cada venta.

En Odoo 19 el PdV no tiene editor de contactos propio: abre la acción
`point_of_sale.res_partner_action_edit_pos` y renderiza la vista form real del
backoffice dentro de un diálogo. Esta capability cubre cómo se personaliza ese
formulario **solo para el PdV**, sin que el formulario de Contactos del
backoffice cambie.
